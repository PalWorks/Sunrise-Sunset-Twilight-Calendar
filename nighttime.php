...
function is_debug(){
	...
	if ( array_key_exists('debug', $_GET) ){ //Test it like this: https://gearside.com/calendars/nighttime.php?debug
		return true;
	}
...
if ( !is_debug() ){
	...
	header('Content-Disposition: attachment; filename=gearside_nighttime.ics');
}
...
X-WR-CALNAME:Gearside - Nighttime<?php echo "\r\n"; ?>
<?php
...
while ( strtotime($date) <= strtotime($year-1 . '-12-31') || strtotime($date) == strtotime($year . '-02-29') ): //The or statement is just for leap days
	...
	$percent = 100+($length*100)/86400;

	list($hours, $minutes) = explode(':', date('G:i', $sunset));
	$startTimestamp = mktime($hours, $minutes);

	$next_day_sun_info = date_sun_info(strtotime('+1 day', $next_year_timestamp), $lat, $lng);
	$next_day_sunrise = $next_day_sun_info['sunrise'];
	list($hours, $minutes) = explode(':', date('G:i', $next_day_sunrise));
	$endTimestamp = mktime($hours, $minutes);

	$seconds = $next_day_sunrise - $sunset;
	...
	if ( !empty($weather_forecast) ){ //If we have weather data
		...
		if ( !empty($weather_forecast[$this_weather_day]) ){ //If we have weather data for this specific day
			...
			if ( !empty($weather_forecast[$this_weather_day]['snow_accumulation']) ){
				...
				// Add extra warning for snow if accumulation is significant
			}
		}
	}
...
DESCRIPTION:<?php echo escape_string(
	$global_notice . " \\n\\n" .
	"Nighttime: " . date('g:ia', $sunset) . ' to ' . date('g:ia', $next_day_sunrise) . " \\n\\n" .
	"Length: " . $hours . "h " . $minutes . "m (" . round($percent, 1) . "%) of nighttime\\n\\n" . // (" . $length_percentile . " percentile)
	$weather_forecast_summary . "\\n\\n" .
...
X-WR-CALDESC:Visualize nighttime<?php echo "\r\n"; ?>
